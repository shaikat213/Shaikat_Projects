...
{
    public class ManyToMayTableDto
    {
    ...
    internal static class ManyToManyDataAssignment
    {
        ...
        static ManyToManyDataAssignment()
        {
            ...
            {
                {"Role", new List<string>{"Task","Menu"}},
                {"User", new List<string>{"Menu","Role"}},
    ...
    internal class BasicDataFacadeUtil
    {
        private readonly string namespaceStr = "Finix.HRM.Infrastructure";
        private readonly GenService _service;

        public BasicDataFacadeUtil(GenService service)
        {
            _service = service;
        }

        public BasicDataFacadeUtil()
        {
        ...
        public List<ManyToMayTableDto> GetMany2ManyTableDataByReflection(string leftTableName, string rightTableName)
        {
            ...
            var leftTableType = assembly.GetType(namespaceStr + "." + leftTableName);
            var rightTableType = assembly.GetType(namespaceStr + "." + rightTableName);
            ...
            var retVal = leftTableData.Select(ld => new ManyToMayTableDto
            {
                IsLeftTable = true,
                Id = ((long)ld.GetType().GetProperty("Id").GetValue(ld, null)),
                Description = GetDescriptionField(ld, leftTableDesciptionFields)
            }).ToList();
            retVal.AddRange(rightTableData.Select(ld => new ManyToMayTableDto
            {
                IsLeftTable = false,
                Id = ((long)ld.GetType().GetProperty("Id").GetValue(ld, null)),
