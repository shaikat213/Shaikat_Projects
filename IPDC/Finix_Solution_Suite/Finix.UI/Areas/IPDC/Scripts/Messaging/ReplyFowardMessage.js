...
$(document).ready(function () {
    ...
    function ReplyForwardMessageVM() {
        var self = this;
        ...
        self.SaveNewMessage = function () {
            ...
            $.ajax({
                type: "POST",
                url: '/IPDC/Messaging/SaveReplyMessage',
        ...
        self.LoadMeassageList = function () {
            self.IpdcMessaging([]);
            var url = '/IPDC/Messaging/LoadMessage?msgId=' + self.RepliedTo() + '&AppId=' + self.ApplicationId();
            return $.ajax({
                type: "GET",
                url: url,
                contentType: "application/json",
                success: function (data) {
                    console.log('from emp-' + ko.toJSON(data.IpdcMessaging));
                    self.IpdcMessaging(data.IpdcMessaging);
                    //self.RepliedTo(data.Id);
                    self.ApplicationId(data.ApplicationId);
                    self.ApplicationNo(data.ApplicationNo);
                    self.AccountTitle(data.AccountTitle);
                    //console.log('from emp-' + ko.toJSON(data.FromEmpId));
                    self.FromEmpId(data.FromEmpId);
                    //console.log('from emp-' + ko.toJSON(data.FromEmpName));
                    self.FromEmpName(data.FromEmpName);
    ...
    appvm = new ReplyForwardMessageVM();
    appvm.Initialize();
    var qValue = appvm.queryString('AppId');
    appvm.ApplicationId(qValue);
    var selfId = appvm.queryString('msgId');
    appvm.RepliedTo(selfId);
    appvm.LoadMeassageList();
    ko.applyBindings(appvm, $('#replyforwardmessage')[0]);
