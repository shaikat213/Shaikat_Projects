...
var DepositApplicationViewModel = function () {
    ...
    self.RateVariance = ko.computed(function () {
        var offer = parseFloat(self.OfferRate() ? self.OfferRate() : 0);
        var card = parseFloat(self.CardRate() ? self.CardRate() : 0);
        total = offer - card;
        return total;
    });
    ...
    self.NomineeCIFPDetails = function (data) {
        var parameters = [{
            Name: 'cifpid',
            Value: data.NomineeCifId()
        }];
        var menuInfo = {
            Id: 27,
            Menu: 'CIF Personal',
    ...
    self.GuiardianCIFPDetails = function (data) {
        var parameters = [{
            Name: 'cifpid',
            Value: data.GuiardianCifId()
        }];
        var menuInfo = {
            Id: 27,
            Menu: 'CIF Personal',
    ...
    self.ApplicantAgeCheck = ko.pureComputed(function () {
        debugger;
        if (self.Application_Id() > 0) {
            console.log(self.ApplicantAge());
    ...
    self.Submit = function () {
        self.FundRealizationDateTxt(moment(self.FundRealizationDate()).format('DD/MM/YYYY'));
        self.MaturityDateTxt(moment(self.MaturityDate()).format('DD/MM/YYYY'));

        var chequeDeposit = ko.observableArray([]);
        var transactionDeposit = ko.observableArray([]);
        var cashDeposit = ko.observableArray([]);
        var nomineees = ko.observableArray([]);

        $.each(self.ChequeDeposits(),
                function (index, value) {
                    chequeDeposit.push({
                        Id: value.Id(),
                        DepositApplicationId: value.DepositApplicationId(),
                        ChequeNo: value.ChequeNo(),
                        ChequeDate: value.ChequeDate(),
                        ChequeDateTxt: value.ChequeDateTxt(),
                        ChequeBank: value.ChequeBank(),
                        DepositedTo: value.DepositedTo(),
                        DepositDate: value.DepositDate(),
                        DepositDateTxt: value.DepositDateTxt(),
                        DepositAmount: value.DepositAmount()
                    });
                });

        $.each(self.TransferDeposits(),
                function (index, value) {
                    transactionDeposit.push({
                        Id: value.Id(),
                        DepositApplicationId: value.DepositApplicationId(),
                        TransferDate: value.TransferDate(),
                        TransferDateTxt: value.TransferDateTxt(),
                        SourceBank: value.SourceBank(),
                        DepositedTo: value.DepositedTo(),
                        DepositAmount: value.DepositAmount()
                    });
                });
        $.each(self.CashDeposits(),
                function (index, value) {
                    cashDeposit.push({
                        Id: value.Id(),
                        DepositApplicationId: value.DepositApplicationId(),
                        ...
                        DepositorPhone: value.DepositorPhone(),
                        CashDepositDate: value.CashDepositDate(),
                        CashDepositDateTxt: value.CashDepositDateTxt(),
                        DepositAmount: value.DepositAmount()
                    });
                });
        ...
        $.each(self.Nominees(),
                function (index, value) {
                    nomineees.push({
                        Id: value.Id(),
                        DepositApplicationId: value.DepositApplicationId(),
                        NomineeCifId: value.NomineeCifId(),
                        RelationshipWithApplicant: value.RelationshipWithApplicant(),
                        PercentageShare: value.PercentageShare(),
                        GuiardianCifId: value.GuiardianCifId(),
                        GuidRelationWithNom: value.GuidRelationWithNom()
                    });
                });

        var submitDepositApp = {
            Id: self.Id(),
            Term: self.Term(),
            Office_Id: self.Office_Id(),
            Application_Id: self.Application_Id(),
            RemovedCashDeposits: self.RemovedCashDeposits(),
            CashDeposits: cashDeposit,
            RemovedChequeDeposits: self.RemovedChequeDeposits(),
            ChequeDeposits: chequeDeposit,
            RemovedTransferDeposits: self.RemovedTransferDeposits(),
            TransferDeposits: transactionDeposit,
            ashDeposits: cashDeposit,
            TotalDepositAmount: self.TotalDepositAmount(),
            ModeOfOperation: self.ModeOfOperation(),
            SpecialInstructions: self.SpecialInstructions(),
            DepositClass: self.DepositClass(),

            CardRate: self.CardRate(),
            OfferRate: self.OfferRate(),
            RateVariance: self.RateVariance(),
            ApprovedBy: self.ApprovedBy(),
            RenewalOpt: self.RenewalOpt(),
            WithdrawalMode: self.WithdrawalMode(),

            CIFAccountTitle: self.CIFAccountTitle(),
            CIFBankAccNo: self.CIFBankAccNo(),
            CIFRoutingNo: self.CIFRoutingNo(),
            CIFBankBranch: self.CIFBankBranch(),
            CIFBank: self.CIFBank(),
            InitialDeposit: self.InitialDeposit(),
            InstallmentSize: self.InstallmentSize(),
            MaturityAmount: self.MaturityAmount(),
            SourceOfFund: self.SourceOfFund(),
            RemovedNominees:self.RemovedNominees(),
            Nominees: nomineees,

            GuiardianCifId: self.GuiardianCifId(),
            RelationshipWithApplicant: self.RelationshipWithApplicant(),
            BenificialOwner: self.BenificialOwner(),
            SourceOfFundDetail: self.SourceOfFundDetail(),
            ...
            FundRealization: self.FundRealization(),
            FundRealizationDate: self.FundRealizationDate(),

            ApplicationStatus: self.ApplicationStatus(),

            TaskAssignedToId: self.TaskAssignedToId(),

            SanctionCheck: self.SanctionCheck(),
            SanctionRemarks: self.SanctionRemarks(),
            AccountOpenDate: self.AccountOpenDate(),
            AccountOpenDateTxt: moment(self.AccountOpenDate()).format('DD/MM/YYYY'),
        ...
        $.ajax({
            url: '/IPDC/Application/SaveDepositApplication',
            //cache: false,
            type: 'POST',
            contentType: 'application/json',
            data: ko.toJSON(submitDepositApp),
            success: function (data) {
                $('#successModal').modal('show');
                $('#successModalText').text(data.Message);
                if (data.Id > 0) {
                    self.LoadDepositAppData();
                }
                    self.Id(data.Id);
            },
            error: function (error) {
    ...
    //        dataType: "json",
    //        success: function (data) {
    //            self.NomineeCif(data); //Put the response in ObservableArray
    //        },
    //        error: function (error) {
    //            alert(error.status + "<--and--> " + error.statusText);
    //        }
    //    });
    //};
    //self.LoadNomineeGuiardianCif = function () {
    //    return $.ajax({
    //        type: "GET",
    //        url: '/IPDC/CIF/GetAllCIFPersons',
    //        contentType: "application/json; charset=utf-8",
    //        dataType: "json",
    //        success: function (data) {
    //            self.GuiardianCif(data); //Put the response in ObservableArray
    //        },
    //        error: function (error) {
    //            alert(error.status + "<--and--> " + error.statusText);
    //        }
    //    });
    //};
    //self.LoadApplicantGuiardianCif = function () {
    //    return $.ajax({
    //        type: "GET",
    //        url: '/IPDC/CIF/GetAllCIFPersons',
    //        contentType: "application/json; charset=utf-8",
    //        dataType: "json",
    //        success: function (data) {
    //            self.AppGuiardianCifList(data); //Put the response in ObservableArray
    //        },
    //        error: function (error) {
    //            alert(error.status + "<--and--> " + error.statusText);
    //        }
