...
$(document).ready(function () {
    ...
    function applicationCIFs(data) {
        ...
        self.ApplicantRole = ko.observable(data ? data.ApplicantRole : 0).extend({
            required: {
                message: "Applicant Role is required",
                onlyIf: function () {
                    return (self.CIF_PersonalId() > 0);

                }
            }
        });
        self.CIF_OrganizationalId = ko.observable(data ? data.CIF_OrganizationalId : '');
        self.CIF_OrganizationalId.subscribe(function () {
            if (self.CIF_OrganizationalId() > 0) {
                self.ApplicantRole(1);
            }
        });
    ...
    function ApplicationVm() {
        ...
        self.IsValid = ko.computed(function () {
            ...
            if (self.CifListValidity().validator === false)
                err++;
            //console.log("err - " + err);
            //console.log("app type - " + self.ApplicationType());
            if (err == 0)
                return true;
            return false;
        });
        ...
        self.SaveApplication = function () {
            ...
            self.ApplicationDateText(moment(self.ApplicationDate()).format('DD/MM/YYYY'));
                ...
                function (index, value) {
                    if (value.IsChecked() === true) {
                        checkListData.push({
                            ...
                            SubmissionDeadlineText: moment(value.SubmissionDeadline()).format('DD/MM/YYYY'),
            ...
            var submitData = {
                ...
                RemovedCIFList: self.RemovedCIFList(),
                ContactPersonAddressType: self.ContactPersonAddressType()
            }
            if (self.IsValid()) {
                $.ajax({
                    type: "POST",
    ...
    //
    $.getScript("/Areas/IPDC/Scripts/Application/depositApplication.js", function () {
        //
    ...
    $.getScript("/Areas/IPDC/Scripts/Application/loanApplication.js", function () {
        //
