...
$(document).ready(function () {
    ...
    function applicationCIFs(data) {
        ...
        self.OccupationTypeName = ko.observable(data ? data.OccupationTypeName : 0);
        ...
        self.LoadData = function (data) {
            ...
            self.OccupationTypeName(data ? data.OccupationTypeName : 0);
            self.checkCoApp();
        }
    ...
    function guarantor() {
        ...
        self.LoadData = function (data) {
            ...
            self.CIFNo(data ? data.CIFNo : 0);
            self.ApplicationId(data ? data.ApplicationId : 0);
            self.RelationshipWithApplicantName(data ? data.RelationshipWithApplicantName : 0);
        }
    ...
    function CRMIndexVm() {
        ...
        self.FullApplication = function () {
            if (self.Id() > 0) {
                var parameters = [{
                    Name: 'applicationId',
                    Value: self.Id()//ko.toJSON(data.Application_Id())
                }];
                var menuInfo = {
                    Id: 127,
                    Menu: 'Application',
        ...
        self.LoadApplicationData = function () {
            //;
            if (self.Id() > 0) {
                $.getJSON("/IPDC/Operations/LoadApplicationByAppIdForCRM/?AppId=" + self.Id(),
                    null,
                    function (data) {
        ...
        self.PrintProposal = function () {
            if (self.ProposalId() > 0) {
                var url = "/IPDC/CRM/CreditMemoReport?reportTypeId=PDF&ProposalId=" + self.ProposalId();
                window.open(url, '_blank');
            }
        }
        self.PrintOfferLetter = function() {
            //if (self.OfferLetterId() > 0) {
            //    var url = "/IPDC/CRM/CreditMemoReport?reportTypeId=PDF&ProposalId=" + self.OfferLetterId();
            //    window.open(url, '_blank');
            //}
            if (self.FacilityType() > 0) {
                if (self.FacilityType() === 2 || self.FacilityType() === 3 || self.FacilityType() === 4) {
                    var url = "/IPDC/CRM/AutoLoanOfferLetterReport?reportTypeId=PDF&OfferLetterId=" + self.OfferLetterId();
                    window.open(url, '_blank');
                } else if (self.FacilityType() === 1) {
                    var url = "/IPDC/CRM/HomeLoanOfferLetterReport?reportTypeId=PDF&OfferLetterId=" + self.OfferLetterId();
                    window.open(url, '_blank');
                }
            }
        }
        self.CreateMemo = function () {
            if (self.Id() > 0 || self.ProposalId() > 0) {
                var parameters = [
                    {
                        Name: 'ApplicationId',
                        Value: self.Id()
        ...
        self.OppApproval = function() {
           
            return $.ajax({
                type: "GET",
                url: '/IPDC/Operations/SaveOprApproval?appId=' + self.Id(),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (data) {
